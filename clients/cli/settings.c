...
/* Available fields for NM_SETTING_TUNNEL_SETTING_NAME */
NmcOutputField nmc_fields_setting_tunnel[] = {
	SETTING_FIELD ("name",  8),                                        /* 0 */
	SETTING_FIELD (NM_SETTING_TUNNEL_MODE, 20),                        /* 1 */
	SETTING_FIELD (NM_SETTING_TUNNEL_LOCAL, 40),                       /* 2 */
	SETTING_FIELD (NM_SETTING_TUNNEL_REMOTE, 40),                      /* 3 */
	SETTING_FIELD (NM_SETTING_TUNNEL_TTL, 20),                         /* 4 */
	{NULL, NULL, 0, NULL, FALSE, FALSE, 0}
};
#define NMC_FIELDS_SETTING_TUNNEL_ALL     "name"","\
                                          NM_SETTING_TUNNEL_MODE","\
                                          NM_SETTING_TUNNEL_LOCAL","\
                                          NM_SETTING_TUNNEL_REMOTE","\
                                          NM_SETTING_TUNNEL_TTL
#define NMC_FIELDS_SETTING_TUNNEL_COMMON  NMC_FIELDS_SETTING_TUNNEL_ALL
...
DEFINE_GETTER (nmc_property_tunnel_get_local, NM_SETTING_TUNNEL_LOCAL);
DEFINE_GETTER (nmc_property_tunnel_get_remote, NM_SETTING_TUNNEL_REMOTE);
DEFINE_GETTER (nmc_property_tunnel_get_ttl, NM_SETTING_TUNNEL_TTL);
...
static char *
nmc_property_tunnel_get_mode (NMSetting *setting, NmcPropertyGetType get_type)
{
	NMSettingTunnel *s_tunnel = NM_SETTING_TUNNEL (setting);
	NMSettingTunnelMode mode;

	mode = nm_setting_tunnel_get_mode (s_tunnel);
	return nm_utils_enum_to_str (nm_setting_tunnel_mode_get_type (), mode);
}

static gboolean
nmc_property_tunnel_set_mode (NMSetting *setting, const char *prop,
                              const char *val, GError **error)
{
	NMSettingTunnelMode mode;
	...
	ret = nm_utils_enum_from_str (nm_setting_tunnel_mode_get_type(), val,
	                               (int *) &mode, NULL);

	if (!ret) {
		values = nm_utils_enum_get_values (nm_setting_tunnel_mode_get_type (),
		                                   NM_SETTING_TUNNEL_MODE_UNKNOWN + 1,
		                                   G_MAXINT);
		options = g_strjoinv (",", (char **) values);
		g_set_error (error, 1, 0, _("invalid option '%s', use one from %s"),
		             val, options);
...
nmc_properties_init (void)
{
	...
	/* Add editable properties for NM_SETTING_TUNNEL_SETTING_NAME */
	nmc_add_prop_funcs (GLUE (TUNNEL, MODE),
	                    nmc_property_tunnel_get_mode,
	                    nmc_property_tunnel_set_mode,
	                    NULL,
	                    NULL,
	                    NULL,
	                    NULL);
	nmc_add_prop_funcs (GLUE (TUNNEL, LOCAL),
	                    nmc_property_tunnel_get_local,
	                    nmc_property_set_string,
	                    NULL,
	                    NULL,
	                    NULL,
	                    NULL);
	nmc_add_prop_funcs (GLUE (TUNNEL, REMOTE),
	                    nmc_property_tunnel_get_remote,
	                    nmc_property_set_string,
	                    NULL,
	                    NULL,
	                    NULL,
	                    NULL);
	nmc_add_prop_funcs (GLUE (TUNNEL, TTL),
	                    nmc_property_tunnel_get_ttl,
	                    nmc_property_set_uint,
	                    NULL,
	                    NULL,
	                    NULL,
...
static gboolean
setting_tunnel_details (NMSetting *setting, NmCli *nmc,  const char *one_prop, gboolean secrets)
{
	NMSettingTunnel *s_tunnel = NM_SETTING_TUNNEL (setting);
	NmcOutputField *tmpl, *arr;
	size_t tmpl_len;

	g_return_val_if_fail (NM_IS_SETTING_TUNNEL (s_tunnel), FALSE);

	tmpl = nmc_fields_setting_tunnel;
	tmpl_len = sizeof (nmc_fields_setting_tunnel);
	nmc->print_fields.indices = parse_output_fields (one_prop ? one_prop : NMC_FIELDS_SETTING_TUNNEL_ALL,
	                                                 tmpl, FALSE, NULL, NULL);
	arr = nmc_dup_fields_array (tmpl, tmpl_len, NMC_OF_FLAG_FIELD_NAMES);
	g_ptr_array_add (nmc->output_data, arr);
	...
	arr = nmc_dup_fields_array (tmpl, tmpl_len, NMC_OF_FLAG_SECTION_PREFIX);
	set_val_str (arr, 0, g_strdup (nm_setting_get_name (setting)));
	set_val_str (arr, 1, nmc_property_tunnel_get_mode (setting, NMC_PROPERTY_GET_PRETTY));
	set_val_str (arr, 2, nmc_property_tunnel_get_local (setting, NMC_PROPERTY_GET_PRETTY));
	set_val_str (arr, 3, nmc_property_tunnel_get_remote (setting, NMC_PROPERTY_GET_PRETTY));
	set_val_str (arr, 4, nmc_property_tunnel_get_ttl (setting, NMC_PROPERTY_GET_PRETTY));
	g_ptr_array_add (nmc->output_data, arr);
...
static const SettingDetails detail_printers[] = {
	...
	{ NM_SETTING_TUNNEL_SETTING_NAME,            setting_tunnel_details },
	{ NULL },
};
