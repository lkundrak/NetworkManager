...
gchar *
@enum_name@_build_string_from_mask (int mask)
{
    guint i;
    gboolean first = TRUE;
    GString *str = NULL;
...
#endif /* __NM_IS_FLAGS__ */

/**
 * @enum_name@_get_value:
 * @str: nickname
 * @out_val: (out) (allow-none) location where to store the value
 *
 * Searches the value matching a given nickname.
 *
...
gboolean @enum_name@_get_value (const gchar *str, int *out_val)
{
    ...
    for (i = 0; @enum_name@_values[i].value_nick; i++) {
        if (!strcmp (str, @enum_name@_values[i].value_nick)) {
            if (out_val)
                *out_val = @enum_name@_values[i].value;
            return TRUE;
	}
    }
