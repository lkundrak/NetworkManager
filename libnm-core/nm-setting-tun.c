...
/**
 * nm_setting_tun_get_user:
 * @setting: the #NMSettingTun
 *
 * Returns: the #NMSettingTun:user property of the setting
 *
 * Since: 1.2
 **/
const char *
nm_setting_tun_get_user (NMSettingTun *setting)
{
	g_return_val_if_fail (NM_IS_SETTING_TUN (setting), NULL);
	return NM_SETTING_TUN_GET_PRIVATE (setting)->user;
}
...
/**
 * nm_setting_tun_get_tap:
 * @setting: the #NMSettingTun
 *
 * Returns: the #NMSettingTun:tap property of the setting
 *
 * Since: 1.2
 **/
gboolean
nm_setting_tun_get_tap (NMSettingTun *setting)
{
	g_return_val_if_fail (NM_IS_SETTING_TUN (setting), FALSE);
	return NM_SETTING_TUN_GET_PRIVATE (setting)->tap;
}
...
verify (NMSetting *setting, NMConnection *connection, GError **error)
{
	...
	if (priv->user) {
		if (_nm_utils_ascii_str_to_int64 (priv->user, 10, 0, G_MAXINT32, -1) == -1) {
			g_set_error (error,
			             NM_CONNECTION_ERROR,
			             NM_CONNECTION_ERROR_INVALID_PROPERTY,
			             _("'%s': invalid user ID"), priv->user);
			g_prefix_error (error, "%s.%s: ", NM_SETTING_TUN_SETTING_NAME, NM_SETTING_TUN_USER);
			return FALSE;
		}
	}
...
nm_setting_tun_class_init (NMSettingTunClass *setting_class)
{
	 ...
	 * NMSettingTun:user:
	 *
	 * The user ID which will own the device, or NULL if only root
	...
	g_object_class_install_property
		(object_class, PROP_USER,
		 g_param_spec_string (NM_SETTING_TUN_USER, "", "",
		                      NULL,
		                      G_PARAM_READWRITE |
	 ...
	 * NMSettingTun:group:
	 *
	 * The group ID which will own the device, or NULL if only the
	 * root group will be able to use it.
	 *
	 * Since: 1.2
	 */
	g_object_class_install_property
	 ...
	 * NMSettingTun:tap:
	 *
	 * If this property is %TRUE the interface will be a TAP device
	 * and will operate with layer 2 (Ethernet) frames. Otherwise
	 * the interface will be a TUN device operating with layer 3 packets.
	 *
	 * Since: 1.2
	 */
	g_object_class_install_property
		(object_class, PROP_TAP,
		 g_param_spec_boolean (NM_SETTING_TUN_TAP, "", "",
		                       FALSE,
		                       G_PARAM_READWRITE |
		                       NM_SETTING_PARAM_INFERRABLE |
		                       G_PARAM_STATIC_STRINGS));
	 ...
	 * physical interface to the data.
	 *
	 * Since: 1.2
	 */
	g_object_class_install_property
	 ...
	 *
	 * If %TRUE the IFF_VNET_HDR will be set on the interface.
	 *
	 * Since: 1.2
	 */
	g_object_class_install_property
