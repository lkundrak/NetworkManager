...
_request_type_to_string (RequestType request_type, gboolean verbose)
{
	...
	case REQUEST_TYPE_P11_GET:  return verbose ? "getting p11 socket" : "p11";
...
struct _NMAgentManagerCallId {
	...
	union {
		...
		struct {
			char *module;
			NMAgentP11SocketResultFunc callback;
			gpointer callback_data;
		} p11;
	};
};
                       ...
                       gpointer user_data)
{
	...
	if (error) {
		if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
			_LOGD (agent, "p11 socket request cancelled: "LOG_REQ_FMT,
			       LOG_REQ_ARG (req));
			return;
		}

		_LOGD (agent, "agent failed p11 socket request "LOG_REQ_FMT": %s",
	...
	_LOGD (agent, "agent returned p11 socket %d for request "LOG_REQ_FMT, fd,
	       LOG_REQ_ARG (req));
...
{
	req->current_call_id = nm_secret_agent_get_p11_socket (req->current,
	                                                       req->p11.module,
...
NMAgentManagerCallId
nm_agent_manager_get_p11_socket (NMAgentManager *self,
                                 NMAuthSubject *subject,
                                 const char *module,
                                 NMAgentP11SocketResultFunc callback,
                                 gpointer callback_data)
{
	...
	nm_log_dbg (LOGD_SETTINGS,
	            "PKCS#11 remoting requested for module %s",
	            module);
	...
	req->p11.module = g_strdup (module);
