...
static void
bluez4_connect_cb (GDBusConnection *dbus_connection,
                   GAsyncResult *res,
                   gpointer user_data)
{
...
static void
bluez5_connect_cb (NMBluez5DunContext *context,
                   const char *device,
                   GError *error,
                   gpointer user_data)
{
                               ...
                               gpointer user_data)
{
	...
	if (connection_bt_type == NM_BT_CAPABILITY_NAP) {
		...
		if (priv->bluez_version == 4)
			...
			priv->b5_context = nm_bluez5_dun_new (priv->adapter_address, priv->address);
			nm_bluez5_dun_connect (priv->b5_context, bluez5_connect_cb, simple);
			return;
		}
	} else
		g_assert_not_reached ();
	...
	g_dbus_connection_call (priv->dbus_connection,
	                        ...
	                        20000,
	                        NULL,
	                        (GAsyncReadyCallback) bluez4_connect_cb,
	                        simple);
}
