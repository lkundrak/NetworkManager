...
update_connection (NMDevice *device, NMConnection *connection)
{
	...
	is_tap = !g_strcmp0 (props.mode, "tap");
	if (is_tap != nm_setting_tun_get_tap (s_tun))
		g_object_set (G_OBJECT (s_tun), NM_SETTING_TUN_TAP, is_tap, NULL);

	user = _nm_utils_ascii_str_to_int64 (nm_setting_tun_get_user (s_tun), 10, 0, G_MAXINT32, -1);
	...
	if (props.owner != user) {
		...
		g_object_set (G_OBJECT (s_tun), NM_SETTING_TUN_USER, str, NULL);
		g_free (str);
	}
...
check_connection_compatible (NMDevice *device, NMConnection *connection)
{
	...
	is_tap = !g_strcmp0 (priv->mode, "tap");
	if (nm_setting_tun_get_tap (s_tun) != is_tap)
		return FALSE;
