...
get_property_neighbors (NMLldpListener *self, GValue *value)
{
	...
	while (g_hash_table_iter_next (&iter, NULL, &ptr)) {
		...
		switch (neigh->dest) {
		case LLDP_DEST_TYPE_NEAREST_BRIDGE:
			dest_str = NM_LLDP_DEST_NEAREST_BRIDGE;
			break;
		case LLDP_DEST_TYPE_NEAREST_NON_TPMR_BRIDGE:
			dest_str = NM_LLDP_DEST_NEAREST_NON_TPMR_BRIDGE;
			break;
		case LLDP_DEST_TYPE_NEAREST_CUSTOMER_BRIDGE:
...
{
	NMLldpListenerPrivate *priv = NM_LLDP_LISTENER_GET_PRIVATE (self);
	sd_lldp_tlv_packet **packets = NULL;
	GHashTable *hash;
	int num, i;

	num = sd_lldp_get_tlv_packets (priv->lldp_handle, &packets, 1024);
	if (num < 0) {
		nm_log_dbg (LOGD_DEVICE, "LLDP: error %d retrieving neighbor TLVs for %s",
		            num, priv->iface);
		return;
	}
	...
	for (i = 0; packets && i < num; i++) {
		...
		r = sd_lldp_tlv_packet_read_chassis_id (packets[i], &chassis_id_type,
		                                        &chassis_id, &chassis_id_len);
		if (r < 0)
			goto next_tlv;

		r = sd_lldp_tlv_packet_read_port_id (packets[i], &port_id_type,
		                                     &port_id, &port_id_len);
		if (r < 0)
			goto next_tlv;
		...
		sd_lldp_tlv_packet_get_destination_type (packets[i],
		                                         (LLDPDestinationType *) &neigh->dest);
		...
		case LLDP_CHASSIS_SUBTYPE_CHASSIS_COMPONENT:
			neigh->chassis_id = strndup ((char *) chassis_id, chassis_id_len - 1);
			break;
		case LLDP_CHASSIS_SUBTYPE_MAC_ADDRESS:
			neigh->chassis_id = nm_utils_hwaddr_ntoa (chassis_id, chassis_id_len - 1);
			break;
		default:
		...
		case LLDP_PORT_SUBTYPE_PORT_COMPONENT:
			neigh->port_id = strndup ((char *) port_id, port_id_len - 1);
			break;
		case LLDP_PORT_SUBTYPE_MAC_ADDRESS:
			neigh->port_id = nm_utils_hwaddr_ntoa (port_id, port_id_len - 1);
			break;
		default:
		...
		if (sd_lldp_tlv_packet_read_port_description (packets[i], &str, &len) == 0) {
		...
		if (sd_lldp_tlv_packet_read_system_name (packets[i], &str, &len) == 0) {
			value = g_slice_new0 (GValue);
			g_value_init (value, G_TYPE_STRING);
			g_value_take_string (value, strndup (str, len));
		...
		if (sd_lldp_tlv_packet_read_system_description (packets[i], &str, &len) == 0) {
			value = g_slice_new0 (GValue);
			g_value_init (value, G_TYPE_STRING);
			g_value_take_string (value, strndup (str, len));
		...
		if (sd_lldp_tlv_packet_read_system_capability (packets[i], &data16) == 0) {
			value = g_slice_new0 (GValue);
		...
		if (sd_lldp_tlv_packet_read_port_vlan_id (packets[i], &data16) == 0) {
			value = g_slice_new0 (GValue);
			g_value_init (value, G_TYPE_UINT);
			g_value_set_uint (value, data16);
		...
		if (sd_lldp_tlv_packet_read_port_protocol_vlan_id (packets[i], &data8, &data16) == 0) {
			value = g_slice_new0 (GValue);
			g_value_init (value, G_TYPE_UINT);
			g_value_set_uint (value, data16);
		...
		if (sd_lldp_tlv_packet_read_vlan_name (packets[i], &data16, &str, &len) == 0) {
			value = g_slice_new0 (GValue);
			g_value_init (value, G_TYPE_UINT);
			g_value_set_uint (value, data16);
...
next_tlv:
		sd_lldp_tlv_packet_unref (packets[i]);
	}
	...
	priv->timer = g_timeout_add_seconds (1, lldp_timeout, self);
