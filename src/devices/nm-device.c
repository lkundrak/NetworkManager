...
dhcp4_cleanup (NMDevice *self, CleanupType cleanup_type, gboolean release)
{
	...
	if (priv->dhcp4_config) {
		nm_exported_object_unexport (NM_EXPORTED_OBJECT (priv->dhcp4_config));
...
dhcp6_cleanup (NMDevice *self, CleanupType cleanup_type, gboolean release)
{
	...
	if (priv->dhcp6_config) {
		nm_exported_object_unexport (NM_EXPORTED_OBJECT (priv->dhcp6_config));
                          ...
                          NMDeviceStateReason *reason)
{
	...
	if (has_changes) {
		...
		if (old_config != priv->ip4_config && old_config) {
			nm_exported_object_unexport (NM_EXPORTED_OBJECT (old_config));
