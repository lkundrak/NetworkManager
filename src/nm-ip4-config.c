...
nm_ip4_config_subtract (NMIP4Config *dst, const NMIP4Config *src)
{
	...
	if (nm_ip4_config_get_mtu (src) == nm_ip4_config_get_mtu (dst))
		nm_ip4_config_set_mtu (dst, 0, NM_PLATFORM_SOURCE_UNKNOWN);
...
void
nm_ip4_config_set_mtu (NMIP4Config *config, guint32 mtu, NMPlatformSource source)
{
	NMIP4ConfigPrivate *priv = NM_IP4_CONFIG_GET_PRIVATE (config);
	...
	} else if (source == priv->mtu_source && priv->mtu > mtu) {
		priv->mtu = mtu;
	}
}
